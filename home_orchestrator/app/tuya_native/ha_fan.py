...
def build_fan(device_id,name,codes,bt):
    ...
    cfg={"name":None,"unique_id":"tuya_%s_fan"%device_id,"device":{"identifiers":["tuya_%s"%device_id],"name":name,"manufacturer":"Tuya"},"availability_topic":bt+"/avail","state_topic":st,"state_value_template":"{{ 'ON' if value_json.%s else 'OFF' }}"%on,"command_topic":cmd+"/"+on,"payload_on":"true","payload_off":"false"}
