...
def build_cover(device_id,name,codes,bt):
    ...
    cfg={"name":name,"unique_id":"tuya_%s_cover"%device_id,"device":{"identifiers":["tuya_%s"%device_id],"name":name,"manufacturer":"Tuya"},"availability_topic":bt+"/avail","command_topic":cmd+"/control","payload_open":"open","payload_close":"close","payload_stop":"stop","state_topic":st,"value_template":"{{ value_json.control_state | default('') }}"}
